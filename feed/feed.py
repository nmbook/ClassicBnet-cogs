...
class ClassicBnetFeed(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.config.register_global(**ClassicBnetFeed.global_conf)
        self.config.register_channel(**{k: v["default"] for k, v in ClassicBnetFeed.channel_conf.items()})
    ...
    async def load_config(self):
        ...
        # reset objects
        self.state = ClassicBnetFeedState()
        self.channel_states = {}

        # store states
        try:
            for key, val in ClassicBnetFeed.global_conf.items():
            ...
            for channel_id, conf in channel_conf.items():
                channel_state = ClassicBnetFeedChannelState()
                for key, val in ClassicBnetFeed.channel_conf.items():
    ...
    def botnet_on_packet(self, packet):
        ...
        if   packet.id == 0x00: # 0x00 S>C keep alive
            ...
            if len(packet) == 4: # end of initial list
                ...
                # save object
                user = ClassicBnetFeedUser(bot_id, bnet_name, bnet_channel, bnet_server, account, database, database_access = dba, admin_access = ada)
    ...
    def send_keep_alive(self): # 0x00 C>S keep alive
        packet = ClassicBnetFeedPacket(id = 0x00)
        return packet
    
    def send_bot_logon(self): # 0x01 C>S bot logon
        packet = ClassicBnetFeedPacket(id = 0x01)
    ...
    def send_self_update(self): # 0x02 C>S self update
        ...
        packet = ClassicBnetFeedPacket(id = 0x02)
    ...
    def send_user_info_list(self): # 0x06 C>S user info list
        packet = ClassicBnetFeedPacket(id = 0x06)
        return packet

    def send_client_caps(self): # 0x0a C>S client caps
        packet = ClassicBnetFeedPacket(id = 0x0a)
    ...
    def send_chat(self, content, *, emote : bool = False, broadcast : bool = False, whisper_to : int = 0): # 0x0b C>S chat
        packet = ClassicBnetFeedPacket(id = 0x0b)
        if whisper_to != 0:
    ...
    def send_account_logon(self, subcommand): # 0x0d C>S account logon
        packet = ClassicBnetFeedPacket(id = 0x0d)
    ...
    def send_chat_opts(self): # 0x10 C>S chat opts
        packet = ClassicBnetFeedPacket(id = 0x10)
    ...
    async def get_packet(self):
        ...
        return ClassicBnetFeedPacket(data = bytearray(header + buf))
                    ...
                    if channel_state.feed_type == "botnet":
                        ...
                        if   action == "JOIN":
                            ...
                            text = self.safe_format(channel_state.join_format,
                                    ClassicBnetFeed.channel_conf["join_format"]["default"], **args)
                        elif action == "PART":
                            args["action"] = "left"
                            text = self.safe_format(channel_state.part_format,
                                    ClassicBnetFeed.channel_conf["part_format"]["default"], **args)
                        else:
                            # ignore user updates and lists
                            return
        ...
        try:
            if channel_state is None:
                ...
                if   action["type"] == "set_key":
                    ...
                    if user_obj is None:
                        # make dummy
                        user_obj = ClassicBnetFeedWebChannelUser(action["event"]["name"], \
    ...
    def rich_text(self, text):
        ...
        repl = []
        for link_match in ClassicBnetFeed.re_discord_link.finditer(text):
        ...
        for em_match in ClassicBnetFeed.re_discord_emoji.finditer(text):
    ...
    def handle_discord_message(self, message, *, length = 496, in_prefix = "", out_prefix = "", out_suffix = "", whisper_to = 0, do_echo_self = False):
        ...
        repl = []
        for em_match in ClassicBnetFeed.re_discord_emoji_object.finditer(clean_content):
    ...
    def handle_webchannel_bncs_userlist(self, channel_state, user, wc_users):
        ...
        channel_wrapper = self.safe_format(channel_state.users_list_format,
                ClassicBnetFeed.channel_conf["users_list_format"]["default"], **list_args)

        n = len(userlist)
        for wc_user in userlist:
            and_n_more = self.safe_format(channel_state.users_list_more_format,
                    ClassicBnetFeed.channel_conf["users_list_format"]["default"],
                    count = n)
            ...
            bnet_inf = self.safe_format(channel_state.users_item_format,
                    ClassicBnetFeed.channel_conf["users_item_format"]["default"], **user_args)
    ...
    def emoji_name(self, text, *, fallback = True):
        ...
        if text in ClassicBnetFeed.emoji_map:
            return ClassicBnetFeed.emoji_map[text]
        else:
            if fallback:
                return ClassicBnetFeed.emoji_map["_"]
            else:
                return text
        ...
        try:
            if self.state.hub_automirror:
                ...
                if hub_guild:
                    ...
                    # set values
                    hub_channel_state = ClassicBnetFeedChannelState()
    ...
    async def botnetget(self, ctx, key : str = "", *, val : str = ""):
        ...
        try:
            ...
            if not key.lower() in ClassicBnetFeed.global_conf:
...
class ClassicBnetFeedChannelState():
    def __init__(self):
        # config mirror
        for key, val in ClassicBnetFeed.channel_conf.items():
...
class ClassicBnetFeedWebChannelUser:
    ...
    def do_parse_text(self):
        try:
            ...
            try:
                self.product_name = ClassicBnetFeed.product_map[self.product]["name"]
                self.product_name_short = ClassicBnetFeed.product_map[self.product]["name_short"]
            except KeyError:
            ...
            if self.product == "DRTL" or self.product == "DSHR":
                ...
                try:
                    self.char_class = ClassicBnetFeed.d1_stats_char_list[self.char_class_id]
                except IndexError:
                ...
                if len(self.text) == 4:
                    ...
                    if len(self.char_data) >= 33:
                        ...
                        try:
                            self.char_class     = ClassicBnetFeed.d2_stats_char_list  [self.char_class_id - 1]
                            self.char_is_female = ClassicBnetFeed.d2_stats_char_female[self.char_class_id - 1]
                        ...
                        try:
                            self.char_title     = ClassicBnetFeed.d2_stats_title_table[(self.char_diff_id, self.char_is_expansion, self.char_is_hardcore, self.char_is_female)]
